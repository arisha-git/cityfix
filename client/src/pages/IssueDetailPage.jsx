...
import {
  FiMapPin, FiClock, FiArrowUp, FiArrowLeft,
  FiUser, FiSend, FiTag, FiCheckCircle
} from 'react-icons/fi'
      ...
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-white rounded-2xl animate-pulse"
            style={{ height: i === 0 ? '320px' : '80px', border: '1px solid #ede9ff' }} />
        ))}
      </div>
    </div>
  )
  ...
  if (!issue) return (
    <div style={{ background: '#f0efff', minHeight: '100vh' }}
      ...
      <div className="text-center">
        ...
        <Link to="/issues" className="text-sm font-bold" style={{ color: '#6C63FF' }}>
          Back to issues
        </Link>
      </div>
    </div>
  ...
  return (
    <div style={{ background: '#f0efff', minHeight: '100vh' }}>
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-sm font-bold mb-8 transition-all"
          style={{ color: '#6C63FF' }}
        >
          <FiArrowLeft size={16} />
          Back to issues
        ...
        {issue.photos?.length > 0 && (
          <div className="mb-6">
            <img
              ...
              style={{ height: '360px', border: '1px solid #ede9ff' }}
            />
        ...
        >
          ...
          <button
            ...
            style={{
              background: hasUpvoted ? '#6C63FF' : '#f0efff',
              color: hasUpvoted ? 'white' : '#6C63FF',
                ...
                <div key={step} className="flex items-center flex-1">
                  <div className="flex flex-col items-center">
                    <div
                      ...
                      style={{
                        background: done ? '#6C63FF' : '#f0efff',
                        color: done ? 'white' : '#9ca3af',
                        border: active ? '3px solid #6C63FF' : '2px solid #ede9ff',
                      }}
                    >
                      {done && !active
                    ...
                    <span
                      ...
                      style={{ color: done ? '#6C63FF' : '#9ca3af' }}
                    >
                      {statusLabels[step]}
                    </span>
                  </div>
                  {i < statusSteps.length - 1 && (
                    <div
                      ...
                      style={{ background: i < currentStep ? '#6C63FF' : '#ede9ff' }}
                    />
                  )}
                </div>
              )
          ...
          {user ? (
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1 flex gap-3">
                  ...
                  <button
                    ...
                    className="px-5 py-2.5 rounded-xl text-white font-bold text-sm flex items-center gap-2 transition-all disabled:opacity-50"
            ...
            >
              <p className="text-sm text-gray-500">
                <Link to="/login" className="font-bold" style={{ color: '#6C63FF' }}>
                  Sign in
                </Link>{' '}
                to leave a comment
              </p>
