...
export default function DashboardPage() {
  ...
  useEffect(() => {
    getIssues()
      .then(res => {
        const mine = res.data.filter(i => i.reportedBy?._id === user?.id ||
    ...
    <div style={{ background: '#f0efff', minHeight: '100vh' }}>
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          {[
            { label: 'Total reported', value: stats.total,      bg: '#f0efff', color: '#6C63FF' },
            { label: 'Open',           value: stats.open,       bg: '#fff0f0', color: '#e53e3e' },
            { label: 'In progress',    value: stats.inProgress, bg: '#fffbeb', color: '#d97706' },
            { label: 'Resolved',       value: stats.resolved,   bg: '#f0fff4', color: '#38a169' },
              ...
              {[...Array(3)].map((_, i) => (
                <div key={i} className="p-5 flex gap-4 items-center">
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="h-4 w-2/3 rounded-lg animate-pulse"
                      style={{ background: '#f0efff' }} />
                    <div className="h-3 w-1/2 rounded-lg animate-pulse"
                      style={{ background: '#f0efff' }} />
                  </div>
                </div>
              ))}
                  ...
                  >
                    ...
                    {issue.photos?.[0] ? (
                      ...
                      <div
                        ...
                        style={{ background: '#f0efff' }}
                      >
                        <FiMapPin size={20} style={{ color: '#6C63FF' }} />
                      </div>
                    )}
                    ...
                    <FiArrowRight size={16} style={{ color: '#c4b5fd', flexShrink: 0 }} />
                  </Link>
                )
              })}
            </div>
